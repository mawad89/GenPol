...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 30 11:44:18 2019
...
def dect_collect(dect,key,mother_dect,region_dect,region):
    ...
    for base in mother_dect[key]:
        ...
        if cc==region_dect[key][region]:
            ...
            break
    return(dect)
variant_list='/netscratch/dep_tsiantis/grp_gan/awad/todo/18_2_2019_vc/canu_nano/29-4-2019/var_call_awad/awad_29_4_2019_with_r_all.vc'
def filterate(variant_list,files=False,output_path=os.getcwd(),output_name='var_call_awad'):
    ...
    for base in p:
        if '*' in base:
            v=int(base.split('\t')[-2].replace('*',''))
            if v==1:
                t.append(base)
                ww=base.split('\t')
                r.append('\t'.join([ww[0]]+[ww[1]]+[str(0)]+[ww[3]]+[ww[4]]+['*']+['\n']))
            else:
                w=0
                ww=base.split('\t')
                x=ww[3]
                ...
                yy=int(ww[7])
                while w<v:
                    t.append('\t'.join([ww[0]]+[str(xx+w)]+[ww[2]]+[x[w]]+[y[w]]+ww[5:7]+[str(yy+w)]+['*1']+[ww[9]]+['\n']))
                    r.append('\t'.join([ww[0]]+[str(xx+w)]+[str(0)]+[x[w]]+[y[w]]+['*']+['\n']))
                    w=w+1
        else:
            t.append(base)
            ww=base.split('\t')
            r.append('\t'.join([ww[0]]+[ww[1]]+[ww[8].replace('+','')]+[ww[3]]+[ww[4]]+['*']+['\n']))
    ...
    for base in q:
        if '*' in base:
            ...
            if v==1:
                u.append(base)
                ww=base.split('\t')
                s.append('\t'.join([ww[0]]+[ww[1]]+[str(0)]+[ww[3]]+[ww[4]]+['*']+['\n']))
            else:
                w=0
                ww=base.split('\t')
                x=ww[3]
                ...
                yy=int(ww[7])
                while w<v:
                    u.append('\t'.join([ww[0]]+[str(xx+w)]+[ww[2]]+[x[w]]+[y[w]]+ww[5:7]+[str(yy+w)]+['*1']+[ww[9]]+['\n']))
                    s.append('\t'.join([ww[0]]+[str(xx+w)]+[str(0)]+[x[w]]+[y[w]]+['*']+['\n']))
                    w=w+1
        else:
            u.append(base)
            ww=base.split('\t')
            s.append('\t'.join([ww[0]]+[ww[1]]+[ww[8].replace('+','')]+[ww[3]]+[ww[4]]+['*']+['\n']))
    if files ==True:
        ...
        for base in q:
            h.writelines(base)
        h.close()
        h=open(output_path+'/'+output_name+'_final_seprated.vc','w')
        for base in t:
            h.writelines(base)
        h.close()
        h=open(output_path+'/'+output_name+'_problematic_seprated.vc','w')
        for base in u:
            h.writelines(base)
        h.close()
        h=open(output_path+'/'+output_name+'.sdi','w')
        for base in r:
            h.writelines(base)
        h.close()
        h=open(output_path+'/'+output_name+'_problematic.sdi','w')
        for base in s:
            h.writelines(base)
        h.close()
    return(p,q)

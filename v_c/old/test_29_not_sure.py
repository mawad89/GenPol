...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 21:27:06 2019
...
def variant_caller(bed_file,ref_genome,draft,output_path=os.getcwd(),output_name='var_call_awad',chr_spliter=' ',contig_spliter=' ',bse=1,qty=30):
    ...
    for base in a:
            ...
            if c[n][-1]=='S' or c[n][-1]=='H':
                ...
                if chr_seg!=contig_seg:
                    ...
                    while q<len(chr_seg):
                        if chr_seg[q]==contig_seg[q]:
                        ...
                        elif chr_seg[q]!=contig_seg[q] and q<len(chr_seg) and chr_seg[q]=='N':
                            qq=0
                            while q<len(chr_seg) and chr_seg[q]!=contig_seg[q] and chr_seg[q]=='N':
                                qq=qq+1
                                q=q+1
                            m=qq
                            ref=''
                            ...
                            while q<len(chr_seg) and chr_seg[q]!=contig_seg[q] and chr_seg[q]!='N':
                                ref=ch1[chr_pos+qq]
                                con=contig[contig_pos+qq]
                                f.append(chr_name+'\t'+str(chr_pos+bse)+'\t'+contig_name+'\t'+ref+'\t'+con+'\t'+quality+'\tSl_1\t'+str(contig_pos+bse)+'\t'+'*'+str(0)+'\t'+strand+'\n')
                                ff.append(chr_name+'\t'+str(chr_pos+bse)+'\t'+contig_name+'\t'+ref+'\t'+con+'\t'+quality+'\tSl_1\t'+str(contig_pos+bse)+'\t'+'*'+str(0)+'\t'+strand+'\n')
                                q=q+1
                                qq=qq+1
                            print(chr_name+'\t'+str(chr_pos+bse)+'\t'+contig_name+'\t'+ref+'\t'+con+'\t'+quality+'\tSl_1\t'+str(contig_pos+bse)+'\t'+'*'+str(0)+'\t'+strand+'\n')
                            contig_pos=contig_pos+qq
                            chr_pos=chr_pos+qq
                            qq=0
                else:
        ...
        while nn<len(fff)-1:
            dd=fff[nn].split('\t')
            ee=fff[nn+1].split('\t')
            if (int(dd[1])+len(dd[3])==int(ee[1])) and (int(dd[7])+len(dd[4])==int(ee[7])) and '*' not in dd[8] and '*' not in ee[8]:

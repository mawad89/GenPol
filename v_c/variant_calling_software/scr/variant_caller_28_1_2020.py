...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 28 18:58:08 2020
...
def variant_caller(bed_file,ref_genome,draft,output_path=os.getcwd(),output_name='var_call_awad',chr_spliter=' ',contig_spliter=' ',bse=1,qty=30):
    ...
    for base in z.keys():
        for ba in z[base]:
            ...
            if ty[0]!='*':
                ...
                counter=0
                while counter<numb:
                    ffff.append(ba.split('\t')[0]+'\t'+str(int(ba.split('\t')[1])+counter)+'\t'+ba.split('\t')[2]+'\t'+ba.split('\t')[3][counter]+'\t'+ba.split('\t')[4][counter]+'\t'+ba.split('\t')[5]+'\t'+ba.split('\t')[6]+'\t'+str(int(ba.split('\t')[7])+counter)+'\t*1\t'+ba.split('\t')[9]+'\n')
                    counter=counter+1
        zz[base]=ffff
        ffff=[]
    ...
    for base in y.keys():
        for ba in y[base]:
            ...
            if ty[0]!='*':
                ...
                counter=0
                while counter<numb:
                    fffff.append(ba.split('\t')[0]+'\t'+str(int(ba.split('\t')[1])+counter)+'\t'+ba.split('\t')[2]+'\t'+ba.split('\t')[3][counter]+'\t'+ba.split('\t')[4][counter]+'\t'+ba.split('\t')[5]+'\t'+ba.split('\t')[6]+'\t'+str(int(ba.split('\t')[7])+counter)+'\t*1\t'+ba.split('\t')[9]+'\n')
                    counter=counter+1
        yy[base]=fffff
        fffff=[]  
    ...
    for base in x.keys():
        for ba in x[base]:
            ...
            if ty[0]!='*':
                ...
                counter=0
                while counter<numb:
                    ffffff.append(ba.split('\t')[0]+'\t'+str(int(ba.split('\t')[1])+counter)+'\t'+ba.split('\t')[2]+'\t'+ba.split('\t')[3][counter]+'\t'+ba.split('\t')[4][counter]+'\t'+ba.split('\t')[5]+'\t'+ba.split('\t')[6]+'\t'+str(int(ba.split('\t')[7])+counter)+'\t*1\t'+ba.split('\t')[9]+'\n')
                    counter=counter+1
        xx[base]=ffffff
        ffffff=[]  
    ...
    for key in x.keys():
        ...
        for base in q:
            h.writelines(base)
    h.close()    
    h=open(output_path+'/'+output_name+'_first_true.vc','w')
    for key in zz.keys():
        q=zz[key]
        for base in q:
            h.writelines(base)
    h.close()
    h=open(output_path+'/'+output_name+'_gather_true.vc','w')
    for key in yy.keys():
        q=yy[key]
        for base in q:
            h.writelines(base)
    h.close()
    h=open(output_path+'/'+output_name+'_with_r_true.vc','w')
    for key in xx.keys():
        q=xx[key]
        for base in q:
            h.writelines(base)

...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 19:09:03 2020
...
def variant_run(bed_file,ref,draft,bed_out_name='map',variant_out_name='calling_variant',output_path=os.getcwd(),software=os.path.dirname(os.path.realpath(__file__))+'/scr/variant_calling_new_bed_12.2.2020.py',filterate=os.path.dirname(os.path.realpath(__file__))+'/scr/filter_04_2_2020.py',chr_spliter="' '",contig_spliter="' '",bse=1,qual=30,q='normal',core=1,job_name='Awad_v_c',submit=False,hits=5000, gatharing_name='_1'):
    ...
    z.writelines('#!/bin/sh -login\nname='+variant_out_name+'\nbed='+os.path.abspath(bed_file)+'\nv_c_file='+variant_out_name+'_first_all.vc'+'\nsoftware='+filterate+'\noutput='+output_path+'\n')
    z.writelines('python $software -b $bed -v $v_c_file -f $name -o $output\n')
    z.close()
    while n<len(a):
        while m<hits and n<len(a):#review
            b.append(a[n])
            n=n+1
            m=m+1
        m=0
        ...
        e.writelines('soft=/biodata/dep_tsiantis/grp_gan/awad/gala/readsep\nread='+draft+'\nread_names=$bed.read_names\n$soft $read $read_names -f fq\n')
        ...
        r=r+1
        b=[]
    d.writelines('bsub -q normal -J '+'gathering_'+ gatharing_name +' -w "done('+"'"+job_name+"'"+')" sh file_assembly.sh')
    d.writelines('\nbsub -q normal -w "done('+"'"+'gathering_'+gatharing_name+"'"+')" sh filter.sh')
    d.close()
    os.system("for i in "+variant_out_name+"/*bed; do cut -f 4 $i> $i.read_names;done;")
    if submit==True:
...
parser.add_argument("-a",nargs=1, type=int, default=[5000], metavar='number_of_hits',dest="hits")
parser.add_argument("-c",nargs=1, type=str, default=['_1'], metavar='gatharing_name_in_job_submission',dest="gatharing_name")
parser.add_argument("-d",nargs=1, type=int, default=[2], metavar='cores',dest="core")
...
hits=args.hits[0]
gatharing_name=args.gatharing_name[0]
core=args.core[0]
variant_run(bed_file=bed,ref=ref,draft=draft,variant_out_name=name,output_path=output,bse=base,qual=qty,q=ch,job_name=gn,submit=submit,hits=hits,gatharing_name=gatharing_name,core=core)

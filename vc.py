...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 15:25:26 2020
...
def variant_caller(bed_file,ref_genome,draft,output_path=os.getcwd(),output_name='var_call_awad',chr_spliter=' ',contig_spliter=' ',bse=1,qty=30):
    ...
    dd={}
    for base in d:
        dd[base.split('\n')[0].split(' ')[0].split('\t')[0]]=''.join(base.split('\n')[1:]).upper()
    d=dd
    dd={}
    e=list(open(draft))
    e=''.join(e)
    if e[0]=='>':
        ...
        co=e[0:7]#somtimes add # before#somtimes 4
        e=e.split(co) # somtimes be @
        del e[0]
        typ='fq'
    del e[0]
    if typ=='fq':
        for base in e:
            dd[co[1:]+base.split('\n')[0].split(' ')[0]]=base.split('\n')[1].upper()#somtimes dell co[1:]
    else:
        for base in e:
            dd[base.split('\n')[0].split('\t')[0]]=''.join(base.split('\n')[1:]).upper()#14.09.2022
    e=dd
    del dd
    s=0
    z={}
    ...
    for base in aa:
        ...
        if 'D' in b or 'I' in b or 'X' in b:
             a.append(base)
    del aa
    starting_1 = timeit.default_timer()
    #print(str(starting))
    for base in a:
        ...
        for n in range(0,len(cigar)):
            ...
            if base == 'D' or base == 'H' or base == 'I' or base == 'M' or base == 'N' or base == 'P' or base == 'S' or base == 'X' or base == '=':
                c.append((int(cigar[m:n]),cigar[n]))
                m=n+1
        ch1=d[chr_name].upper()
        contig=e[contig_name].upper()
        if strand=='-':
            contig=rc(contig)
        n=0
    ...
    h.writelines('time=\t'+str(stoping-starting_1))
    h.close()

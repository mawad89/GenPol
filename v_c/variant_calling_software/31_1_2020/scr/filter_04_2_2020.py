...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  6 20:31:18 2020
...
def filteration(bed,vc_file,output_path=os.getcwd(),output_name='var_call_awad'):
    ...
    chr_name=list(z.keys())[0]#del it in the update
    ...
    for key in x.keys():
        ...
        for base in q:
            if abs(float(base.split('\t')[-1][:-1]))>1:#7.2.2020
                fff.append(base)
            elif int(base.split('\t')[-5])<0 and abs(float(base.split('\t')[-2]))>=0.5:#6.4.2020
                fff.append(base)
    ffff={}
    for bas in fff:
        ffff[bas.split('\t')[1]]=[]
    ...
    for bas in ffff.keys():
        if len(ffff[bas])==1:
            ...
            for nec in fffff:#6.4.2020
                if abs(float(nec.split('\t')[-2]))>=0.5:
                    fff.append(nec)
    y[chr_name]=fff
    ...
    for key in y.keys():
        q=y[key]
        q=sorted(q, key=lambda i:(int(i.split('\t')[1]),int(i.split('\t')[4].replace('*1','1').replace('+1','0').replace('-1','2'))))#7.4.2020
        for base in q:
            h.writelines(base)
